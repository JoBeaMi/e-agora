...
1. Em github.com/JoBeaMi, criar um repositório novo chamado `e-agora` (público, vazio, sem README).
2. No Terminal do Mac (ou no terminal do RStudio), dentro da pasta `livro/`:

    git init -b main
    git add .
    git commit -m "Livro da UC: estrutura"
    git remote add origin https://github.com/JoBeaMi/e-agora.git
...
A partir daqui, cada `git push` para `main` renderiza e publica sozinho (separador Actions mostra o progresso, 2–3 minutos). O livro fica em https://joanamiguel.pt/e-agora/ (e também em https://jobeami.github.io/e-agora/ até o domínio estar ligado).
...
repositório com Pages aparecer em joanamiguel.pt/<nome-do-repositório>. Não pôr "Custom domain" no repositório e-agora.
